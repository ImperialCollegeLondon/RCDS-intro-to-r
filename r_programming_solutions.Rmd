...
In linear algebra, a _rotation matrix_ is used to perform rotations in Euclidean space.
...
\end{bmatrix}
$$
rotates a 2D vector $x$ anticlockwise by the angle $\theta$: 
$$
x' = Mx
$$

Find the result of rotating the vector (5,3) anticlockwise by 60$^{\circ}$.
```{r}
x <- c(5,3)
theta <- (60/360)*2*pi
...
A good example of a categorical variable is sex. In many circumstances we can limit the sex categories to ”Male” or ”Female”:
```{r}
...
Which indices of `tempi` indicate [tempi](https://en.wikipedia.org/wiki/Tempo#Basic_tempo_markings) greater than 108bpm?
```{r}
tempi <- c("Adagio", "Largo", "Moderato", "Andante", "Adagio", "Allegro", "Largo", "Presto", "Moderato", "Adagio")

table(factor(tempi))

tempi <- factor(tempi, ordered=TRUE, levels = c("Largo", "Adagio", "Andante", "Moderato", "Allegro", "Presto"))
which(tempi >= "Moderato")
```
...
Bad design of loops is the most common source of inefficient R code.

Whenever you’re using a `for` loop, you might want to revise your code and see whether you can use one of the `apply` family functions
instead. These are set of compact loop commands for a variety of data structures. Here we will look at the variants `lapply`, `sapply` and `tapply`.

### `lapply` {-}

The simplest `apply` family function is `lapply`:

```
lapply(X, FUN, ...)
```

`lapply` takes a vector or list `X`, and applies the function `FUN` to each of its elements. 
If `FUN` requires additional arguments, you need to pass them after you’ve specified `X` and `FUN` (indicated as `...` in the function definition above).
The output of `lapply()` is a *list* of the same length as `X`, where each element is the result of applying `FUN` to the corresponding element of `X`.

First let's select some continuous-valued columns from the planets data.
```{r}
planets.subset <- planets[3:7]
planets.subset
```

Now we can use `lapply` to find the means:
```{r}
lapply(planets.subset, mean)
```
Note that `lapply` coerces the data frame to a list.


### `sapply` {-}

`sapply` is a useful variant of `lapply` which will attempt to simplify the output to a vector or matrix where possible, otherwise it returns a list.
```{r}
sapply(planets.subset, mean)
```

Compare the `sapply` output (a named vector) with the `lapply` output above (a list).

If the output of `FUN()` is a vector, `lapply` makes a list of vectors, which `sapply` interprets as a matrix:
```{r}
lapply(planets.subset, round, digits=1)
```

```{r}
sapply(planets.subset, round, digits=1)
```

### `tapply` {-}

`tapply` is another `apply` family function which can be useful for working with data frame columns. It has the form

```
tapply(X, INDEX, FUN, ...)
```

Here, `INDEX` is a list of factors which is the same length as `X`. The input data `X` will be grouped according to the levels of `INDEX` before applying `FUN`.
```{r}
tapply(planets$diameter, planets$type, mean)
```


#### Exercise {-}

Can you construct a new data frame containing the numerical attributes for an "average" planet of each of the three types? (Hint: make a matrix first and then coerce to a data frame using `as.data.frame()`)
```{r}
# add the density column
planets.subset2 <- cbind(planets.subset, planets["density"]) 

# a function to get the medians for a particular column, grouped by type
get.avg <- function(x){
  tapply(x, planets$type, median)
}

# use sapply to construct a matrix
avg.mat <- sapply(planets.subset2, get.avg)
avg.mat <- round(avg.mat,2)

# coerce to a data frame
avg.df <- as.data.frame(avg.mat)
avg.df
```
...
Find the average BMI for each baseball position. (Hint: use `tapply()`)

```{r}
mean.BMI <- tapply(mlb$BMI, mlb$Position, mean)
mean.BMI
```
...
The box plot is nice, but we want to tidy it up for a presentation. 
Can you re-order the boxes from smallest to largest average BMI? (Hint: use an ordered factor)
```{r}
# We previously calculated the mean BMI for each Position
mean.BMI

# from smallest to largest mean BMI, the order of Positions is
order(mean.BMI)

# or in terms of Position names,
names(mean.BMI)[order(mean.BMI)]

# We can use this ordering of levels to rewrite the Position column as an ordered factor:
mlb$Position <- factor(mlb$Position, ordered = TRUE, levels=names(mean.BMI)[order(mean.BMI)])

# Finally, we redraw the box plot.
boxplot(mlb$BMI ~ mlb$Position, las=2, horizontal=TRUE, cex.axis=0.5, xlab="BMI", ylab="Position")
```
